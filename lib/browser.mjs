...
export function killOrphanedChrome() {
  try {
    ...
    execSync(
      `ps aux | grep "remote-debugging-port" | grep -v grep | awk '{print $2}' | xargs -r kill -15 2>/dev/null; true`,
      { encoding: "utf8", timeout: 5000 }
    );
    // Give them a moment to exit cleanly, then SIGKILL survivors
    execSync(
      `sleep 1 && ps aux | grep "remote-debugging-port" | grep -v grep | awk '{print $2}' | xargs -r kill -9 2>/dev/null; true`,
      { encoding: "utf8", timeout: 5000 }
    );
  } catch {}
}
